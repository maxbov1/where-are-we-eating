...
The root frontend URL opens the landing page, with the primary CTA leading to
`event-creation.html`. The organizer flow continues to `survey-creation.html`
after organizer setup, then opens `share-event.html`, where the organizer can
scan a QR code or copy the guest link and group-chat message. Existing guest
invitation links remain compatible: `/?survey=<token>` is handled by the legacy
guest response route in `index.html`.
