...
def test_recommendation_credential_error_preserves_cors_headers(monkeypatch):
    """Browser clients must receive the API error instead of a fake CORS failure."""
    aggregate = {
        "survey_id": "survey-1",
    ...
    monkeypatch.setattr(api, "run", lambda *args, **kwargs: (_ for _ in ()).throw(NoCredentialsError()))
