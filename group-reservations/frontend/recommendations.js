...
function renderActions(actions) {
  $('booking-handoff').innerHTML = actions.map((action) => action.url ? `<a class="button secondary" href="${escapeHtml(action.url)}" target="_blank" rel="noreferrer">${escapeHtml(action.label)} ↗</a>` : `<button class="text-button" type="button" data-action="${escapeHtml(action.id)}">${escapeHtml(action.label)}</button>`).join('');
