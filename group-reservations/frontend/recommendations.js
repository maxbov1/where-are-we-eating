...
function renderContract(result) {
  ...
  const conversation = (result.conversation || []).map((message) => `<p class="agent-message agent-message-${escapeHtml(message.role || 'assistant')}">${escapeHtml(message.content || '')}</p>`).join('');
  const option = (item, primary) => `<article class="recommendation-card ${primary ? 'recommendation-primary' : ''}"><div class="card-kicker">${primary ? 'Best fit for the group' : 'Another option'}</div><h3>${item.restaurant_url ? `<a class="recommendation-name" href="${escapeHtml(safeUrl(item.restaurant_url))}" target="_blank" rel="noreferrer">${escapeHtml(item.name)} ↗</a>` : escapeHtml(item.name)}</h3><p class="recommendation-description">${escapeHtml(item.description || '')}</p>${item.traits?.length ? `<p class="recommendation-traits">${item.traits.map((trait) => escapeHtml(trait)).join(' · ')}</p>` : ''}${item.tradeoff ? `<p class="recommendation-tradeoff">${escapeHtml(item.tradeoff)}</p>` : ''}<p class="recommendation-availability"><strong>${item.availability?.status === 'verified' ? 'Availability verified' : 'Availability not verified'}:</strong> ${escapeHtml(item.availability?.summary || 'Unknown')}</p>${item.reservation?.url ? `<a class="recommendation-action" href="${escapeHtml(safeUrl(item.reservation.url))}" target="_blank" rel="noreferrer">${escapeHtml(item.reservation.label || `Get ${item.name}'s reservation`)} ↗</a>` : '<span class="recommendation-unavailable">Reservation path not verified</span>'}</article>`;
