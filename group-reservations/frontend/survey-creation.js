...
function renderScheduleEditor(values = fridayDates()) { $('schedule-editor').innerHTML = values.slice(0,3).map((date,index) => `<div class="schedule-editor-row"><label>Date ${index + 1}<input required type="date" class="schedule-date" value="${date}" /></label><div class="schedule-times"><div class="schedule-times-heading"><span>Available times</span><small>Up to 3 for this date</small></div><div class="schedule-time-list"><label class="time-row"><input required class="time-input" type="time" value="18:00" /><button class="remove-time" type="button" aria-label="Remove time">×</button></label></div><button class="add-time" type="button">+ Add another time</button></div></div>`).join('') + (values.length < 3 ? '<button class="add-date" type="button">+ Add another date</button>' : ''); }
