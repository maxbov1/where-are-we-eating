...
function fridayDates() { const today = new Date(); const friday = new Date(today); friday.setDate(today.getDate() + ((5 - today.getDay() + 7) % 7 || 7)); return [0,7,14].map((offset) => { const date = new Date(friday); date.setDate(friday.getDate() + offset); return date.toISOString().slice(0,10); }); }
...
function renderScheduleEditor(values = fridayDates()) { $('schedule-editor').innerHTML = values.map((date,index) => `<div class="schedule-editor-row"><label>Date ${index + 1}<input required type="date" class="schedule-date" value="${date}" /></label><div class="schedule-times"><div class="schedule-times-heading"><span>Available times</span><small>Up to 3 for this date</small></div><div class="schedule-time-list">${['18:00','19:00','20:00'].map((time) => `<label class="time-row"><input required class="time-input" type="time" value="${time}" /><button class="remove-time" type="button" aria-label="Remove time">×</button></label>`).join('')}</div><button class="add-time" type="button">+ Add another time</button></div></div>`).join(''); }
