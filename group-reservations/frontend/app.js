...
function selectedTopics() { return [...document.querySelectorAll('#question-topics input:checked')].map((input) => input.value); }
function renderQuestionOptions() {
  $('question-options').innerHTML = selectedTopics().map((key) => `<section class="question-option-group"><div class="question-option-heading"><strong>${QUESTION_LABELS[key]}</strong><span>Choose what guests can see</span></div><div class="option-toggle-grid">${questionState[key].map((option, index) => `<label class="option-toggle"><input type="checkbox" data-question="${key}" data-option-index="${index}" checked /> <span>${option}</span></label>`).join('')}</div></section>`).join('');
}
...
$('question-options').addEventListener('change', (event) => {
  ...
  const enabled = [...document.querySelectorAll(`input[data-question="${key}"]:checked`)].map((input) => questionState[key][Number(input.dataset.optionIndex)]);
  if (!enabled.length) event.target.checked = true;
});
